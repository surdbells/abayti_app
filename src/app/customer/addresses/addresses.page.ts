...
@Component({
  ...
  standalone: true,
  imports: [IonContent, IonHeader, IonTitle, IonToolbar, CommonModule, FormsModule, IonButtons, IonCard, IonCardContent, IonFab, IonFabButton, TuiButton, TuiIcon, TuiLoader, RouterLink, IonCardHeader, IonCardTitle, IonCardSubtitle]
})
export class AddressesPage implements OnInit, OnDestroy {
  ...
  add_billing() {
    this.router.navigate(['/', 'create_billing']).then(r => console.log(r));
  }
  get_billing() {
  ...
  get_selected(name: string, id: number) {
    this.router.navigate(['/', 'update_billing'],{ queryParams: { id: id, name: name } }).then(r => console.log(r));
  }
